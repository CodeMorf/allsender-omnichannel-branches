...
export function createBranchPublicRouter({ apiKeyService, externalRecordService, Branch, Knowledge }) {
  ...
  router.get('/profile', authorize('branch.read'), async (req, res) => {
    const branch = await Branch.findOne({ _id: req.branchId, workspace_id: req.branchWorkspaceId, deleted_at: null, status: 'active' }).select('name code description address location timezone languages default_language phone email coverage_mode status').lean();
  ...
  router.put('/records/:type/:externalId', authorize((req) => writeScopeFor(req.params.type)), async (req, res) => {
    const data = await externalRecordService.upsert({ workspaceId: req.branchWorkspaceId, branchId: req.branchId, type: req.params.type, externalId: req.params.externalId, payload: req.body?.payload ?? req.body, externalUpdatedAt: req.body?.external_updated_at, source: 'branch_api', customerRefs: req.body?.customer_refs || {}, accessMode: req.body?.access_mode || 'customer_bound' });
    return res.json({ success: true, data });
  });
  router.get('/knowledge', authorize('branch.knowledge.read'), async (req, res) => {
    const data = await Knowledge.find({ workspace_id: req.branchWorkspaceId, deleted_at: null, status: 'active', $or: [{ scope: 'workspace' }, { branch_id: req.branchId }] }).select('scope type title language country version effective_date content source_url updated_at').sort({ scope: 1, title: 1 }).lean();
    return res.json({ success: true, data });
  });
