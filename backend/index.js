...
export function createBranchesModule({ host }) {
  ...
  const branchService = new BranchService({ Branch, Membership, BranchAgent, Knowledge, Integration });
  ...
  const workspaceMiddleware = async (req, res, next) => {
    try {
      const workspaceId = await host.resolveWorkspaceId(req);
      if (!workspaceId) return res.status(403).json({ success: false, message: 'Selecciona un espacio de trabajo válido.' });
      req.branchWorkspaceId = workspaceId;
      return next();
    } catch (error) {
      console.error('[branches:workspace]', error);
      return res.status(403).json({ success: false, message: 'No se pudo validar el espacio de trabajo.' });
    }
  };
  ...
  return {
    models,
    services: { branchService, geoService, resolver, conversationControl, integrationService, handoffService, apiKeyService, externalRecordService, agentRuntime, openApiImportService },
    workspaceMiddleware,
    router,
    publicRouter
  };
...
export {
  Branch,
  Membership as BranchMembership,
  BranchAgent,
  Knowledge as BranchKnowledge,
  Integration as BranchIntegration,
  ConversationState as BranchConversationState,
  Handoff as BranchHandoff,
  BranchApiKey,
  ExternalRecord as BranchExternalRecord
};
