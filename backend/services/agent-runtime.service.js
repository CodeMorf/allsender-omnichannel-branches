...
export class BranchAgentRuntime {
  constructor({ models, host, conversationControl, handoffService, integrationService }) {
    this.models = models;
    this.host = host;
  ...
  async searchKnowledge({ workspaceId, branchId, query, language }) {
    const base = {
      ...
      deleted_at: null,
      status: 'active',
      $or: [{ scope: 'workspace' }, { branch_id: branchId }]
    };
    if (language) base.$and = [{ $or: [{ language: null }, { language }, { language: '' }] }];
    let rows = [];
    try {
      rows = await this.models.Knowledge.find({ ...base, $text: { $search: query } }, { score: { $meta: 'textScore' } })
        .sort({ score: { $meta: 'textScore' } }).limit(6).lean();
    } catch {
      const escaped = String(query).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      rows = await this.models.Knowledge.find({ ...base, $or: [{ title: { $regex: escaped, $options: 'i' } }, { content: { $regex: escaped, $options: 'i' } }] }).limit(6).lean();
    }
    return rows.map((row) => ({ title: row.title, type: row.type, language: row.language, content: String(row.content || '').slice(0, 3500), source_url: row.source_url }));
  }
  ...
  buildTools({ workspaceId, branch, branchAgent, conversationKey, channelContext, language }) {
    ...
    if (allowed.has('branch_info')) tools.push(createTool({
      name: 'branch_info',
      description: 'Get verified information about the current business branch.',
      parameters: z.object({}),
      execute: async () => publicBranch(branch)
    }));

    if (allowed.has('knowledge_search')) tools.push(createTool({
      name: 'knowledge_search',
      description: 'Search verified company and branch knowledge, policies, terms, products and service information. Use this instead of inventing business facts.',
      parameters: z.object({ query: z.string().min(2) }),
      execute: async ({ query }) => this.searchKnowledge({ workspaceId, branchId: branch._id, query, language })
    }));

    if (allowed.has('external_api')) tools.push(createTool({
      name: 'external_business_api',
      description: 'Read verified information from an approved branch integration, such as order status, tracking, inventory or reservations.',
      parameters: z.object({ integration: z.string(), operation: z.string(), params: z.record(z.string(), z.any()).default({}) }),
      execute: async ({ integration, operation, params }) => this.integrationService.execute({ workspaceId, branchId: branch._id, integrationCode: integration, operationCode: operation, params, allowWrite: false })
    }));

    if (allowed.has('request_human') && branchAgent.handoff_enabled) tools.push(createTool({
      name: 'request_human',
      description: 'Transfer the conversation to a human agent at this branch. Use when the customer asks for a person, the issue requires human authorization, or reliable resolution is not possible.',
      parameters: z.object({ reason: z.string().default('customer_request'), summary: z.string().min(1), priority: z.enum(['low','normal','high','urgent']).default('normal') }),
      execute: async ({ reason, summary, priority }) => {
        const handoff = await this.handoffService.request({ workspaceId, branchId: branch._id, conversationKey, language, reason, summary, priority, channelContext });
        return { transferred: true, handoff_id: String(handoff._id), message: branchAgent.handoff_message };
      }
    }));
  ...
  async respond({ workspaceId, branchId, conversationKey, message, language = null, agentId = null, channelContext = {}, metadata = {} }) {
    ...
    const tools = this.buildTools({ workspaceId, branch, branchAgent, conversationKey, channelContext, language: language || lock.language });
    ...
    await this.models.ConversationState.findOneAndUpdate(
      ...
      {
        $set: { language: language || latest?.language || null, branch_id: branchId, ...(latest?.responder_type === 'AI' ? { responder_since: latest?.responder_since || now } : {}) },
        $push: { recent_messages: { $each: [{ role: 'customer', content: String(message), at: now }, ...(text ? [{ role: 'assistant', content: text, at: now }] : [])], $slice: -20 } }
      },
      { upsert: true }
    );
